...
__global__ void histogramKernelv3(
    ...
    if (i * stride + stride - 1 < N) [likely](likely) {
        #pragma unroll
...
The `#pragma unroll` directive instructs the compiler to unroll the loop that loads consecutive elements. The `[likely](likely)` attribute hints that this branch of the conditional is expected to be executed most of the time, allowing the compiler to optimize it further. If the array length is not a multiple of `stride`, the remaining elements are processed one by one. This version achieves an execution time of approximately **11.136 µs**.
